...
def report_inventory_turnover():
    ...
    try:
        ...
        end_str     = request.args.get("end_date")
        try:
            start = date.fromisoformat(start_str) if start_str else today - timedelta(days=30)
        except ValueError:
        ...
        cur.execute("""
            SELECT
                ...
                COALESCE(b.brand_name, 'Generic')              AS brand_name,
                COALESCE(u.uom_name, '—')                      AS uom,
                COALESCE(stock.ending_qty, 0)                  AS ending_qty,
            ...
            JOIN static_status    ss_i ON ss_i.status_id = i.item_status_id
            JOIN static_status    ss_b ON ss_b.status_id = ib.item_status_id
            -- ending stock: current qty on hand
            LEFT JOIN (
                SELECT inventory_brand_id,
                       SUM(quantity_on_hand) AS ending_qty
            ...
            ) stock ON stock.inventory_brand_id = ib.inventory_brand_id
            -- units sold + COGS in period
            LEFT JOIN (
                SELECT bat.inventory_brand_id,
                ...
                JOIN order_transaction ot ON ot.order_id = od.order_id
                JOIN sales_transaction st ON st.order_id = ot.order_id
                WHERE od.is_archived = FALSE
                  AND st.sales_date BETWEEN %s AND %s
                GROUP BY bat.inventory_brand_id
            ) sales ON sales.inventory_brand_id = ib.inventory_brand_id
            WHERE ss_i.status_code != 'INACTIVE'
              AND ss_b.status_code != 'ARCHIVED'
            ORDER BY COALESCE(sales.cogs, 0) DESC, i.item_name
        """, (start, end))
        rows = cur.fetchall()

        result = []
        for r in rows:
            ending_qty   = int(r[4] or 0)
            units_sold   = int(r[5] or 0)
            cogs         = float(r[6] or 0)
            # beginning = ending + units sold (units sold were removed from stock during period)
            beginning_qty = ending_qty + units_sold
            avg_inventory = (beginning_qty + ending_qty) / 2.0
            turnover      = round(cogs / avg_inventory, 2) if avg_inventory > 0 else 0.0
            result.append({
                "sku":             r[0],
                "item_name":       r[1],
                "brand_name":      r[2],
